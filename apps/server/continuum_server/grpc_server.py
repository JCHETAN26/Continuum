...
class EmbedServiceServicer(embed_pb2_grpc.EmbedServiceServicer):
    async def EmbedBatch(self, request, context: grpc.aio.ServicerContext) -> embed_pb2.EmbedBatchResponse:
        texts = list(request.texts)
        if len(texts) > 32:
            context.abort(grpc.StatusCode.INVALID_ARGUMENT, "Batch size exceeds maximum of 32")
            
        try:
            embeddings_list, version, dim = await engine.embed_batch(texts)
